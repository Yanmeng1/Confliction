...
public class ConflictModel {
    ...
    /** 1. 初始化Agent(location \ asset \ culture) **/
    ...
    /** 3. 核心逻辑1：相互作用相互影响
     *              创建下一代（location信息浅拷贝，包括加载邻居）
     *              局部经济变化
     *              宏观经济影响
     *              文化交融
     *              **/
    public Agent[][] nextAgents() {
        /* 1. 创建下一代agent(location, neighborLocation) */
        ...
        /* 5 基于asset和culture更新 economicDissatisfy, cultureDifferent  */
        for (int row = 0; row < nextAgents.length; row ++) {
            for (int col = 0; col < nextAgents[row].length; col ++) {
                Agent nextAgent = nextAgents[row][col];
    ...
    /** 4. 核心逻辑2：身份转变(结合)
     *              周围恐怖分子诱导；
     *              经济差异大的情况下文化与敌对势力综合影响诱导；
     *              display信息(0-1满意)(<0暴乱)(0-1)资产分布
     *              **/
        ...
        for (int row = 0; row < rows; row ++) {
            for (int col = 0; col < columns; col ++) {
                ...
                if (agent.isRioter()) {
                    ...
                    /* 恐怖主义激活: 1.局部恐怖分子诱导 2.不满意冲破阈值，文化和外部敌对势力综合作用agent */
